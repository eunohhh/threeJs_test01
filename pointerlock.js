...
class PointerLockControls extends EventDispatcher {
    ...
    onTouchMove(e) {
        ...
        case 2:
            if (e.touches[0].target === this.domElement.querySelector('canvas')) touch = e.touches[0]
            else if (e.touches[1].target === this.domElement.querySelector('canvas')) touch = e.touches[1]
            break
        }
        if (!touch) return
    ...
    onPointerlockChange() {
        if (this.domElement.ownerDocument.pointerLockElement === this.domElement.querySelector('canvas')) {
